<?php
// Database update script for PCG CG-12 Course Management System
$host = "localhost";
$user = "root";
$pass = "";
...
$sql_file = file_get_contents('update_courses_database.sql');

// Split SQL file into individual queries
$queries = explode(';', $sql_file);

echo "<!DOCTYPE html>";
echo "<html><head><title>Database Update - PCG CG-12 Courses</title>";
echo "<style>
...
echo "<h1>PCG CG-12 Course Database Update</h1>";
echo "<p>Updating course structure with Local, Foreign, and Unit/Interagency Training categories</p>";
echo "</div>";
...
foreach ($queries as $index => $query) {
    ...
    if (!empty($query) && !preg_match('/^--/', $query)) {
        ...
        if ($conn->query($query) === TRUE) {
            ...
            // Count INSERT statements for courses
            if (stripos($query, 'INSERT INTO courses') !== false) {
                $total_courses_added++;
            }
        } else {
...
echo "<h3>Database Update Summary</h3>";
echo "<p><strong>Successful operations:</strong> $success_count</p>";
echo "<p><strong>Errors:</strong> $error_count</p>";
echo "<p><strong>Total courses added/updated:</strong> $total_courses_added</p>";
...
if ($error_count == 0) {
    ...
    echo "The course database has been updated with the new structure including:<br>";
    echo "• Local Training courses (Career, Functional, Senior Level)<br>";
    echo "• Foreign Training courses<br>";
    echo "• Unit/Interagency Training courses<br>";
    echo "• Enhanced course details and categorization";
    echo "</div>";
} else {
